...
/* Please contact Chen.Tang@UTSouthwestern.edu for programming questions */
...
// Compute non-edge forces using Barnes-Hut algorithm
void SPTree::computeNonEdgeForces_SpaSNE(unsigned int point_index, double theta, double neg_f[], double* sum_Q, double neg_f_global[], double* sum_Q_global)
{
    ...
    if(is_leaf || max_width / sqrt(D) < theta) 
    {
        ...
        D_global = 1.0 / D; 
        double mult = cum_size * D;
        double mult_global = cum_size * D_global; 
        *sum_Q += mult;
        *sum_Q_global += mult_global; 
        mult *= D;
        mult_global *= D;
        for(unsigned int d = 0; d < dimension; d++) 
        {
        	neg_f[d] += mult * buff[d];
        	neg_f_global[d] += mult_global * buff[d]; 
        }
    }
    else 
    {
        // Recursively apply Barnes-Hut to children
        for(unsigned int i = 0; i < no_children; i++) 
        	children[i]->computeNonEdgeForces_SpaSNE(point_index, theta, neg_f, sum_Q, neg_f_global, sum_Q_global);
    }
}
...
void SPTree::computeEdgeForces_SpaSNE(int N, double* pos_f, double* P, double* P_global, double* P_space, double alpha, double beta)
{
    double D;
    ...
    for(unsigned int n = 0; n < N; n++) 
    {
        ...
        for(unsigned int m = 0; m < N; m++)
	    {
	        ...
	        for(unsigned int d = 0; d < dimension; d++) 
	        	buff[d] = data[nD + d] - data[mD + d];
	        	
	        for(unsigned int d = 0; d < dimension; d++) 
	        	D += buff[d] * buff[d];
	        	
	        D = (P[nN + m] - alpha * P_global[nN + m] - beta * P_space[nN + m]) / D;
