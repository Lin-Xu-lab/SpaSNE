...
```
The above command has been successfully tested on two Linux operating systems. 

After the environment is successfully created, you can use the following command
to activate it:
```
...
g++ 11.2.0 version. Note, if the g++ version is too old, it may not successfully
compile the code, and then you need to "module load" or install a newer g++ 
version. After you set up the environment, please run the following command in 
the terminal to generate the "spasne" executable file.
```
gmake
```
