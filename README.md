...
If the "git clone" command does not work with your computer, you can download 
the zip file from the website 
https://github.com/Lin-Xu-lab/SpaSNE.git and uncompress it. Then, the folder 
...
set up the above environment, please run the following command in the terminal 
to generate the "spasne" executable file: 
```
gmake
```
Please see the Makefile file for other commands. If your computer does not have
GNU Make installed, please use the following command in the terminal to
generate the "spasne" executable file: 
```
g++ sptree.cpp spasne.cpp spasne_main.cpp -o spasne -O2
``` 

Note, if you do have the above environment, the software package may also work 
for the environment of your machine. That is why the __Compilation__ part is 
placed before __Environment setup__. You can use the two examples in 
__Examples__ for testing. Normally, both should be completed in a short time. 
However, if you find the code get stuck there, please follow the instructions in
__Environment setup__ to set up the environment. 

__Environment setup__

An option to set up the environment is to use 
Conda 
...
Below is example 1.1 for the mouse visual Cortex data. First, please type
```
cd spasne-examples
```
