...
3.8.8, pandas version 1.3.4, and g++ version 11.2.0. An option to set up the 
environment is to use Conda 
...
There are two SpaSNE examples in the "spasne-examples" folder.
```
cd spasne-examples
```
5.1.1. Please use jupyter notebook to open the 
     spasne_VisualCortex1207_example.ipynb for the Mouse Visual Cortex example.

5.1.2. Please use jupyter notebook to open the 
     spasne_BreastCancer1272_example.ipynb for the Human Breast Cancer example.
  
__5.2. Preprocessing examples__

There is one preprocessing example in the "preprocessing-example" folder. 
```
cd preprocessing-example
```
5.2.1. Please use Jupyter notebook to open the 
...
__5.3. A script example__
	
Below is example 5.1.1 for the mouse visual Cortex data. First, please type
```
cd spasne-examples
```
