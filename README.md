...
a comprehensive low-dimensional visualization that best reflects the molecular 
similarities of cells and the spatial interactions between cells.  
...
For random initialization, we noticed that the magnitude of 𝛼 influences the 
reproducibility of the embedding. A larger 𝛼 results in a higher chance of 
embedding failure, especially for small (spatial samples) datasets. For example, 
for the BreastCancer1272 dataset used as an example in this software package, we
found there was a higher chance of embedding failure when setting 𝛼 to 12 than 
setting it to 10 (𝛽 was also changed accordingly based on an 𝛼 / 𝛽 ratio). 
Therefore, for small datasets, it might be relatively safe when setting 𝛼 not 
larger than 10. 
...
Please contact C. Tang for programming questions about *.cpp and spasne.py files.
